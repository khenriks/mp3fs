...
#else
static inline void talloc_set_name_v(const void *ptr, const char *fmt, va_list ap) {};
inline void talloc_set_name(const void *ptr, const char *fmt, ...){};
inline void talloc_set_name_const(const void *ptr, const char *name){};
inline void *talloc_named(const void *context, size_t size, const char *fmt, ...)
{
  return NULL;
};

inline const char *talloc_get_name(const void *ptr){
  return NULL;
};

inline void *talloc_check_name(const void *ptr, const char *name){
  return NULL;
};
...
#ifdef WINDOWS
	while(1) {
	  len = vsnprintf(s, s_len, fmt, ap2);
	  if(len >0 ) break;

	  s_len+=256;
	  s = talloc_realloc(NULL, s, char, s_len);
	  if (!s) return NULL;
	};
#else
	len = vsnprintf(NULL, 0, fmt, ap2);
